...
export function formatComment(report: CompatibilityReport): string {
  ...
  comment += `- **Blocking Issues:** ${blockingCount > 0 ? `❌ ${blockingCount}` : '✅ 0'}\n`;
  comment += `- **Warnings:** ${warningCount > 0 ? `⚠️ ${warningCount}` : '✅ 0'}\n\n`;

  if (results.length === 0) {
    comment += `✅ No web features detected in this PR that require compatibility checking.\n`;
    return comment;
  }
  ...
  comment += `\n---\n`;
  comment += `🤖 *Generated by [GreenLightCI](https://github.com/your-org/greenlightci)*\n`;
...
function formatFeatureResult(result: CompatibilityResult): string {
  ...
  // Links
  if (feature.mdnUrl) {
    output += `- 📖 [MDN Documentation](${feature.mdnUrl})\n`;
  }
